...
On launch, the app briefly shows a dedicated Blazares Orpheus splash screen before entering the tuner. It uses the same near-black graphite background and neon green/cyan precision language as the main interface. A luminous, stylized figure of Orpheus playing a lyre is centered inside a subtle animated ring and waveform pulse. The BLAZARES / ORPHEUS wordmark sits below the figure. The transition should feel intentional and musical, remain short enough not to delay use, and avoid showing an empty or unrelated platform splash.
